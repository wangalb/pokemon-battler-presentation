...
Every turn in that middle screen is a real round trip to the Java backend. Speed decides who strikes first, damage comes from power, attack over defense, same-type bonus, critical hits, and the shared type chart, and a faint ends the battle. The frontend displays the returned result rather than reimplementing those rules.`,
  ...
  `ResolveTurnInteractor is the Use Case Interactor for a single battle. Its responsibility is to order one turn: reject a battle that is already complete, find the move the player chose, ask the move selector for the opponent's move, decide who acts first, and return one TurnResult. It also coordinates the case where the first attack ends the battle and the second attack must not occur.

The arithmetic is not inside the Interactor. BattleSimulator owns accuracy, critical hits, type effectiveness, and damage, because those enterprise rules would still be true if the game were called from another interface. Battle, Pokemon, Move, Bag, and TurnResult protect valid combat state. Keeping coordination separate from arithmetic gives each class a smaller reason to change.

The diagram maps the same path to Clean Architecture. The React arena and web gateway are Frameworks and Drivers. BattleController is an Interface Adapter. ResolveTurnInputBoundary, its data structures, and the Interactor are Application Business Rules. The combat objects are Entities. Runtime control goes from the View inward and the result returns outward, while source-code dependencies point toward the inner policies.

Dependency Inversion is visible at the boundary. BattleController depends on ResolveTurnInputBoundary, never on the concrete Interactor. The Interactor realizes that boundary. OpponentMoveSelector is a Strategy, and Random is injected through the constructor. Tests can therefore choose the opponent move, accuracy roll, and critical-hit roll. That makes the same production rules deterministic without adding test-only branches.`,
