...
We also use cache-aside for official data. CachedPokeApiDataAccess checks local cached data, falls through to PokeAPI on a miss, then stores the result. The use cases still depend on narrow data-access interfaces, so caching does not leak into their rules. Across these examples, the patterns solve concrete problems: selectable behaviour, object assembly, and avoiding repeated remote calls.`,
  ...
  `This slide is our proof of testing coverage. The current verification passed 309 Java tests and 325 TypeScript tests, for 634 total. JaCoCo reports 90.96 percent backend line coverage, and Vitest reports 89.80 percent frontend line coverage. The use-case layers are above 95 percent in both stacks, which matters because that is where application coordination lives.
...
CI runs Checkstyle with the repository's MyStyle rules for Java conventions, including named constants, Javadoc, and single-exit methods. Oxlint checks TypeScript and React. CI also runs backend and frontend tests, coverage gates, production builds, dependency checks, and architecture rules. A failed job means the change is not ready to merge. The Maven Wrapper permission is checked because CI must execute the same build from a clean Linux checkout.
